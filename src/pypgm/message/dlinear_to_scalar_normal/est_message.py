def est_message(from_factor_parameters, to_factor_parameters,
    ...
    """Estimate message parameters from dlinear factor to scalar_normal factor
    Args :
    ...
    if set(out_message_parameters['vars']) == set(from_factor_parameters['output_vars']):
        #parameter_set = dlinear.kl_optimization_iow(from_factor_parameters, inplace=True)
        parameter_set = dlinear.kl_optimization_o(from_factor_parameters, inplace=True)

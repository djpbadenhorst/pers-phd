...
class test_pypgm(unittest.TestCase):
    ...
    def test_01(self):
        ...
        self.pgm.connect_fnodes('f(y)', 'f(x,y,w,p)', ['y'])
    ...
    def test_03(self):
        ...
        message_before = self.pgm.get_message_parameters('f(y)','f(x,y,w,p)')

        inc_message = self.pgm.get_message_parameters('f(x,y,w,p)','f(y)')
        inc_message['wmean'] = 0.1
        inc_message['prec'] = 1
        self.pgm._set_value('f(x,y,w,p)->f(y)', inc_message)
        
        self.pgm.update_message('f(y)', 'f(x,y,w,p)')
        message_after = self.pgm.get_message_parameters('f(y)', 'f(x,y,w,p)')

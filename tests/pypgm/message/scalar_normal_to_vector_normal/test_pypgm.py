...
class test_pypgm(unittest.TestCase):
    ...
    def test_01(self):
        ...
        self.pgm.connect_fnodes('f(x)', 'f(x,y)', ['x'])
    ...
    def test_02(self):
        ...
        belief_before = self.pgm.get_belief_parameters('f(x,y)')
        self.pgm.update_belief('f(x,y)')
        belief_after = self.pgm.get_belief_parameters('f(x,y)')
    ...
    def test_03(self):
        ...
        message_before = self.pgm.get_message_parameters('f(x)','f(x,y)')

        inc_message = self.pgm.get_message_parameters('f(x,y)','f(x)')
        inc_message['wmean'] = 0.1
        inc_message['prec'] = 0.1
        self.pgm._set_value('f(x,y)->f(x)', inc_message)
        
        self.pgm.update_message('f(x)', 'f(x,y)')
        message_after = self.pgm.get_message_parameters('f(x)','f(x,y)')

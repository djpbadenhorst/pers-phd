...
class test_logistic_regression(unittest.TestCase):
    ...
    def test(self):
        ...
        Y_VEC = X_MATRIX.dot(W_TRUE)
        Z_VEC = 1./(1+np.exp(-Y_VEC)) + np.random.normal(0, Z_NOISE, NUMBER_SAMPLES)
        ...
        def init_pgm():
            ...
            for cnt in range(NUMBER_SAMPLES):
                ...
                pgm.add_fnode(fname('f', ['z',cnt]),
                              ftype=FACTOR_TYPE.SCALAR_NORMAL,
                              par_form=scalar_normal.PARAMETER_FORM.STANDARD,
                              vars=fnode_z_vars,
                              mean=Z_VEC[cnt],
                              var=Z_VAR)
                pgm.add_fnode(fname('f', ['y',cnt], ['z',cnt]),
                              ftype=FACTOR_TYPE.DSIGMOID,
                              input_vars=fnode_y_vars,
                              output_vars=fnode_z_vars)
        ...
        def connect_fnodes():
            ...
            for cnt in range(NUMBER_SAMPLES):
                ...
                fnode_y_vars = ['y_'+padstr(cnt)]
                fnode_z_vars = ['z_'+padstr(cnt)]
                dlinear_factor_name = fname('f', ['x',cnt], ['y',cnt], ['w'], ['p'])
                dsigmoid_factor_name = fname('f', ['y',cnt], ['z',cnt])
                pgm.connect_fnodes(fname('f', ['x',cnt]), dlinear_factor_name, fnode_x_vars)
                pgm.connect_fnodes(fname('f', ['y',cnt]), dlinear_factor_name, fnode_y_vars)
                pgm.connect_fnodes(fname('f', ['y',cnt]), dsigmoid_factor_name, fnode_y_vars)
                pgm.connect_fnodes(fname('f', ['z',cnt]), dsigmoid_factor_name, fnode_z_vars)
                pgm.connect_fnodes('f(w)', dlinear_factor_name, fnode_w_vars)
                pgm.connect_fnodes('f(p)', dlinear_factor_name, fnode_p_vars)
        ...
        def message_passing():
            if SHOW_IMAGES:
                ...
                plt.xticks(np.arange(len(W_EVAL_VALUES)), np.round(np.linspace(min(W_EVAL_VALUES), max(W_EVAL_VALUES), len(W_EVAL_VALUES)),1))
                plt.yticks(np.arange(len(W_EVAL_VALUES)), np.round(np.linspace(min(W_EVAL_VALUES), max(W_EVAL_VALUES), len(W_EVAL_VALUES)),1))
                plt.savefig('./images/examples/sae_w_' + padstr(0))
                plt.close()

                modval_belief = pgm.get_belief_parameters('f(p)')
                plt.title('Prior')
                plt.plot(M_EVAL_VALUES, gamma.calculate_pdf(M_EVAL_VALUES, **modval_belief))
                plt.savefig('./images/examples/sae_p_' + padstr(0))
                plt.close()
            ...
            for counter in range(NUM_ITERATIONS):
                ...
                for cnt in range(NUMBER_SAMPLES):
                    dlinear_factor_name = fname('f', ['x',cnt], ['y',cnt], ['w'], ['p'])
                    dsigmoid_factor_name = fname('f', ['y',cnt], ['z',cnt])
                    pgm.update_message(dlinear_factor_name, fname('f', ['y', cnt]))
                    pgm.update_belief(fname('f', ['y', cnt]))
                    pgm.update_message(fname('f', ['y', cnt]), dsigmoid_factor_name)
                    pgm.update_belief(dsigmoid_factor_name)
                    pgm.update_message(dsigmoid_factor_name, fname('f', ['y', cnt]))
                    pgm.update_belief(fname('f', ['y', cnt]))
                    pgm.update_message(fname('f', ['y', cnt]), dlinear_factor_name)
                ...
                if SHOW_IMAGES:
                    ...
                    plt.xticks(np.arange(len(W_EVAL_VALUES)), np.round(np.linspace(min(W_EVAL_VALUES), max(W_EVAL_VALUES), len(W_EVAL_VALUES)),1))
                    plt.yticks(np.arange(len(W_EVAL_VALUES)), np.round(np.linspace(min(W_EVAL_VALUES), max(W_EVAL_VALUES), len(W_EVAL_VALUES)),1))
                    plt.savefig('./images/examples/sae_w_' + padstr(counter+1))
                    plt.close()

                    modval_belief = pgm.get_belief_parameters('f(p)')
                    plt.title('Belief')
                    plt.plot(M_EVAL_VALUES, gamma.calculate_pdf(M_EVAL_VALUES, **modval_belief))
                    plt.savefig('./images/examples/sae_p_' + padstr(counter+1))
                    plt.close()
